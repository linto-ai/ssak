...
def rename_audios_based_on_wavscp(input_folder, char_to_replace, replacement_char):
    with open(os.path.join(input_folder, "wav.scp"), "r") as f:
        ...
        for line in tqdm(content):
            ...
            file_path = line[1]
            new_file = file_path.split("/")[-1].replace(
...
def replace_char_in_file(
    file_path, char_to_replace, replacement_char, column=-1, delimiter=" ", slices=1
):
    new_content = ""
    if slices is None:
        slices = -1
    ...
    if os.path.exists(file_path):
        print(
            f"File '{file_path}' has been modified ('{char_to_replace}' -> '{replacement_char}')"
        )
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Replace a character in utterances or audio_ids (or both) of a kaldi dataset."
    )
    parser.add_argument("input_folder", type=str, help="Input folder with kaldi files")
    parser.add_argument(
        "--char_to_replace", type=str, default=":", help="Character to replace"
    )
    parser.add_argument(
        "--replacement_char", type=str, default="-", help="Replacement character"
    )
    parser.add_argument(
        "--delimiter", type=str, default=" ", help="Delimiter for kaldi files"
    )
    parser.add_argument(
        "--rename_wavs",
        action="store_true",
    ...
    if args.rename_wavs:
        rename_audios_based_on_wavscp(
            args.input_folder, args.char_to_replace, args.replacement_char
        )
