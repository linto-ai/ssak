...
from sak.utils.text_ar import *
from sak.utils.text_latin import *
...
def whisper_infer(model, dataloder, forced_decoder_ids, device):
    ...
    # mode="reduce-overhead" mode uses CUDA graphs to further reduce the overhead of Python.
    ...
    for _, batch in enumerate(tqdm(dataloder)):
        # Ensure input_features are assigned properly from the batch
...
if __name__=='__main__':
    ...
    parser.add_argument("model_path_or_size", help="You can put a local path of fine-tuned model or a size of openai model [base, tiny, small, medium....]", default="small")
    parser.add_argument('--language', "-l", help=f"Language to use. Among : {', '.join(sorted(k+'('+v+')' for k,v in whisper.tokenizer.LANGUAGES.items()))}.", default = "fr")
    ...
    base_size = ['tiny.en', 'tiny', 'base.en', 'base', 'small.en', 'small', 'medium.en', 'medium', 'large-v1', 'large-v2', 'large-v3', 'large']
    ...
    wer, prediction, references = whisper_infer(
                                                model=model, 
                                                dataloder=eval_dataloader, 
                                                forced_decoder_ids=forced_decoder_ids, 
                                                device=device
                                                )
