...
def torchaudio_is_valid(model):
    return isinstance(model, tuple) \
        and len(model) == 2 \
        and isinstance(model[0], torchaudio.models.wav2vec2.model.Wav2Vec2Model) \
        and isinstance(model[1], tuple)
    ...
    with torch.inference_mode():
        for audio in audios:
            ...
            if l > max_len:
                ...
                print("Audio too long, splitting into {} chunks for alignment".format(math.ceil(l / max_len)))
                logits = []
