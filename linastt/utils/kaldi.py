...
def check_kaldi_dir(dirname, language=None, strict_sort=True):
    strict_sort = str(strict_sort).lower() == "true"
    ...
    p = subprocess.Popen([tool_dir + "/fix_data_dir.sh", dirname, str(strict_sort).lower()])
    p.communicate()
    if p.returncode != 0:
    ...
    p = subprocess.Popen([tool_dir + "/validate_data_dir.sh", "--no-feats", "" if strict_sort else "--no-spk-sort", dirname])
    p.communicate()
    if p.returncode != 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--strict_sort', help='If sort on speakers must be equal to sort on utterances', default=True)
