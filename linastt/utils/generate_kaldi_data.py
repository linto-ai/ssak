...
def generate_kaldi_data(audio_folder, transcription_folder, output_folder, extension='.mp3'):
    ...
    with open(segments_file, "w", encoding='utf-8') as segments, \
        ...
        for audio_file in os.listdir(audio_folder):
            expected_extensions = [".mp3", ".wav", ".ogg"]
            if not os.path.splitext(audio_file)[1] in expected_extensions:
                warnings.warn(f"Ignoring {audio_file} because it is not in the expected format.", UserWarning)
                continue
            ...
            with open(transcription_path, "r") as f:  
                ...
                for row in reader:
                    ...
                    segments.write(f"{audio_name}-seg_{utt_id} {audio_name} {start:.2f} {end:.2f}\n")
                    wav_scp.write(f"{audio_name} sox {audio_path} -t wav -r 16000  -b 16 -c 1 - |\n")
                    utt2spk.write(f"{audio_name}-seg_{utt_id} {audio_name}-{utt_id}\n")
                    spk2utt.write(f"{audio_name}-seg_{utt_id} {audio_name}-{utt_id}\n")
                    spk2gender.write(f"{audio_name}-seg_{utt_id} M\n")
                    utt2dur.write(f"{audio_name}-seg_{utt_id} {duration}\n")
                    txt.write(f"{audio_name}-seg_{utt_id} {text}\n")
                    utt_id += 1
